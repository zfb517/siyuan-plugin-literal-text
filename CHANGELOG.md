...
| 版本    | 日期         | 变更                                                                   |
| ----- | ---------- | -------------------------------------------------------------------- |
| 2.8.14 | 2026-09-22 | 修复集市 issue #1（macOS 上 Cmd+Shift+L 无法关闭/被兜底监听覆盖）：兜底 keydown 监听器不再硬编码 ⇧⌘E/L/V，改为实时读取用户在「设置→快捷键」的生效绑定（忠实复刻思源 getKeymapBindings / matchHotKey 语义），仅当用户实际绑定被按下才触发，尊重改绑/删除；用户清空某命令绑定后该命令兜底不再触发 |
